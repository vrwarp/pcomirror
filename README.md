...
service says which pages may read its answers. So name them:

```sh
PCOMIRROR_CORS_ORIGINS=https://directory.yourchurch.org,https://*.yourchurch.org
```

**Off unless that is set, and off is silent** — no `Access-Control-*` header
anywhere and `OPTIONS` stays the `405` it already was, which is exactly what a
browser needs to see to conclude the service is not for it. There is no
permissive default, because no default could be both useful and safe for a mirror
of a church's people database.
...
startup** rather than never matching. A wildcard replaces the first label only:
...
an `X-Mirror-Cors` header naming the variable to change:
...
# X-Mirror-Cors: origin 'https://elsewhere.example' is not in PCOMIRROR_CORS_ORIGINS
```

`/admin` shows the policy in force under **Browser access**, and `serve` prints it
at startup. Both are read-only: unlike the divergence rate and the subscription
list, which an operator has to change mid-incident, who may read the mirror from
a browser is a deployment decision and not one to make from a web page.
...
- **Browser access** — the CORS policy in force: which origins, methods and
  headers, and how long a preflight is cached. Read-only; see
...
```sh
python3 run_tests.py     # 598 end-to-end tests + 11 writer-semantics assertions
```
...
request-supplied text cannot reach a response header.
