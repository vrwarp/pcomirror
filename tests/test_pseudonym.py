...
class TestNobodyRealSurvives(unittest.TestCase):
    ...
    def test_an_unclassified_attribute_is_redacted_not_passed_through(self):
        ...
        self.assertEqual(p.value("some_field_invented_next_year", "whatever it holds"),
                         values.REDACTED)

    def test_free_text_is_never_fabricated(self):
        """A plausible-looking medical note is worse than an obvious placeholder."""
        self.assertEqual(P().value("medical_notes", "Carries an EpiPen"), values.REDACTED)
    ...
    def test_identifiers_that_point_outside_pco_are_redacted(self):
        ...
        for attribute in ("remote_id", "login_identifier", "stripe_customer_identifier"):
            self.assertEqual(p.value(attribute, "abc-123"), values.REDACTED)
...
class TestTheFakesAreBelievable(unittest.TestCase):
    ...
    def test_a_malformed_value_is_redacted_rather_than_repaired(self):
        ...
        p = P()
        self.assertEqual(p.value("address", "not-an-email"), values.REDACTED)
        self.assertEqual(p.value("birthdate", "9999-99-99-nonsense"), values.REDACTED)
...
class TestTheSecret(unittest.TestCase):
    def test_it_is_minted_once_and_then_reused(self):
        m, _ = build()
        first = pseudonym.secret_for(m.db)
        self.assertEqual(pseudonym.secret_for(m.db), first)
